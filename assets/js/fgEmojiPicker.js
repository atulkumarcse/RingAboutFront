                ...
                fetchData = new Promise((resolve, reject) => {
                    ...
                    for (const key in object) {
                        if (object.hasOwnProperty(key)) {
                            ...
                            categoriesInner += `<li class="${index === 1 ? '' : ''}" id="${keyToId}" data-index="${keyToId}"><a href="#${keyToId}">${this.variable.categoryIcons[keyToId]}</a></li>`

                            innerLists += `
                            <ul class="fg-emoji-picker-category ${index === 1 ? '' : ''}" id="${keyToId}" category-name="${key}">
