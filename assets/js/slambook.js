    ...
    var apiurl = "https://steponexp.net/IHMdata/";
       ...
       //setInterval(getmsg,5000);
         const emojiPicker = new FgEmojiPicker({
            ...
            emit(obj, triggerElement) {
                $('#mytextareaslam').val($('#mytextareaslam').val()+obj.emoji);
                var textval = $('#mytextareaslam').val();
                globaltextarea = textval 
                //console.log("hi",globaltextarea)
                if ((textval) != null) {
                  cmax = textval.length;
                }
                ...
                if (cmax > words) {
                  ...
                  var trimmed = $('#mytextareaslam').val($('#mytextareaslam').val().substr(0, words));
                } else {
                    //words = words-2;
    ...
    function saveDetails()
{
    ...
    $.ajax({
              url: apiurl+'/api/savenotification',
              type: "GET",
...
$(".sup-send").on("click",async function(){
    var supporid = await areadCookie("supporid");
        if(supporid == "" || typeof(supporid) === undefined || supporid == null || supporid == "null")
        {
           saveSupportDetails();
        }
    ...
    function submitform()
         ...
         //   e.preventDefault();
            ...
            var data = {username:username,password:rollno}
...
$('#mytextareaslam').keyup(function(e){
    if(e.keyCode == 8){
        // textarray =  globaltextarea.split(/(?!$)/u);
        // ele = textarray[textarray.length-1];
        // // if(ele.length > 1){
        // //     cmax = cmax - 2
        // //     cmax = globaltextarea.length
        // // }
        cmax = globaltextarea.length
       if(words-cmax >= 100){
         $('#display_count').text(100)
         cmax = 0;
         words =100;
       } else {
        $('#display_count').text((words-cmax))
    ...
    globaltextarea = $('#mytextareaslam').val();
    if (event.keyCode == 13)
    {
           var msg = $('#mytextareaslam').val();
            msg = msg.trim();
...
'@keyframes lblinks{0%{opacity:0.3;}50%{box-shadow: 0px 0px 1px 7px #283b79;opacity: 1;}100%{opacity: 0;}}'+
...
function modalpopup(){
    ...
    $("#msupport").modal("show");  
     getmessagesupport(); 
     logs('','support_modal_popup',166);
    }
}

$(".photob3save-btn").on("click",function(){
    logs('','save_photobooth',277);
});

$(".save-btn").on("click",function(){
    logs('','class_photobooth',577);
});
...
// Calculates minutes until 8pm
  const
    ...
    eightPM = new Date();
  eightPM.setHours(19, 20, 0, 0); // Sets the new date object's time to 8pm
  const
  ...
  // console.log(`Minutes until 8pm: ${minsDuration}`);
   if(window.location.href.indexOf("auditorium") > 11) // This doesn't work, any suggestions?
    {
        if(minsDuration < -15  && minsDuration > -17){
         clearInterval(timerd);
         $("lblink").css("display","none");
         logs('','removeaudi',22);
       window.location.reload();
     }
     else if(minsDuration<0 && minsDuration > -17) // This doesn't work, any suggestions?
    { 
    ...
    {
        if(minsDuration<0 && minsDuration > -15) // This doesn't work, any suggestions?
    {
...
var notytimer = ""
 function noty(){
    if(notytimer == 900000 || notytimer == "900000")
    {
        message= "Please Download the Yearbook";
        type = 1;
        savenotification(message,type)
        toastr.info(message);
        $(".notificationblink").addClass("blink");
       notytimer = 0;
                            ...
                            notytimer = Math.floor(notytimer)+1000;
                            createCookie("notytimer",notytimer)
                        };

       async function notyfy(){
            notytimer = await readCookie("notytimer");
            noty();

        }     
setInterval( notyfy , 1000);
